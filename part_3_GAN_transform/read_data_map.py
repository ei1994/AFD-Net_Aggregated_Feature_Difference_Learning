...
#    filename = os.path.join(SAVA_PATH, TRAIN_FILE)
    filename = 'map_data/map_country.tfrecord'
        ...
        try:
            for i in range(100):
                ...
                for j in range(img.shape[0]):
                    misc.imsave(str(j)+'_read_rgb.png', np.squeeze(img[j,:,:64]))
                    misc.imsave(str(j)+'_read_nir.png', np.squeeze(img[j,:,64:]))
