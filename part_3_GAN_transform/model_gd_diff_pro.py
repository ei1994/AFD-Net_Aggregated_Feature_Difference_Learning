...
def matching(feature_map_1, feature_map_2, feature_map_3): 
    with tf.variable_scope('match_layers'):
#       特征图求差值
        diff_feature_1 = tf.abs(feature_map_1 - feature_map_2)
        diff_feature_2 = tf.abs(feature_map_3 - feature_map_2)
        all_feature = tf.concat([diff_feature_1,diff_feature_2], axis=3)  # 两个特征连接 4*4*512
...
def create_discriminator(inputs_p1, inputs_p2, feature_map_3, name="discriminator", reuse=False):
    with tf.variable_scope(name, reuse=reuse):
        ...
        output = matching(feature_map_1, feature_map_2, feature_map_3)   
        return output
...
def gd_model_g2r(inputs_g, inputs_r):
    # gray --> nir
        ...
        with tf.name_scope("real_discriminator"):
            ...
            feature_map_3, layers_3 = nir_encode(inputs_r)
            predict_real = create_discriminator(inputs_g, inputs_r, feature_map_3 )
        with tf.name_scope("fake_discriminator"):
            # 2x [batch, height, width, channels] => [batch, 1]
            feature_map_3, layers_3 = nir_encode(g_outputs, reuse=True)
            predict_fake = create_discriminator(inputs_g, g_outputs, feature_map_3, reuse=True)
