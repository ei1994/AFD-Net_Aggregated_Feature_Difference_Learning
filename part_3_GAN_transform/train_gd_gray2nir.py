...
def gd_train(f_name):
    checkpoint_dir = 'ckpt_gd_g2r_'+f_name
    checkpoint_dir_g = 'ckpt_g_g2r_'+f_name
    output_dir = 'out_g2r_'+f_name
    checkpoint_file = os.path.join(checkpoint_dir, 'model.ckpt')
    checkpoint_file_g = os.path.join(checkpoint_dir_g, 'model.ckpt')
    initLogging('record_gd_g2r_'+f_name+'.log')
...
#                                mc_test_all(current_epoch)
                ...
                np.save(os.path.join(checkpoint_dir,'ckpt_map_gd_'+f_name), all_loss)
                np.save(os.path.join(checkpoint_dir_g,'ckpt_map_gd_'+f_name), all_loss)
...
#    numbers_train = train_matching_y.shape[0]  #训练集总数
    ...
    with graph.as_default():
        ...
        with tf.Session() as sess:
            sess.run(tf.local_variables_initializer())
#            saver.restore(sess, tf.train.latest_checkpoint('ckpt_gd_g2r_all'))
            saver.restore(sess, 'ckpt_gd_g2r_all/model.ckpt-'+ str(number))
#            num = 0
            ...
            try:
                ...
                while not coord.should_stop():
                    if step_test < 1:
                        ...
                        gen_out_dir = 'out_g2r_all/epoch_' + str(number)
                        try:
