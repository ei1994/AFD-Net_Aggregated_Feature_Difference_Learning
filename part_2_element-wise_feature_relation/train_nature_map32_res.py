...
import math
import time
import model_d_map32_res as model
import read_data
...
checkpoint_dir = 'ckpt_map32_res'
checkpoint_file = os.path.join(checkpoint_dir, 'model.ckpt')
train_dir='summary'

def initLogging(logFilename='record_map32_res.log'):
  """Init for logging
  """
  logging.basicConfig(
                    level    = logging.DEBUG,
    ...
    with graph.as_default():
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            try:
                ...
                np.save(os.path.join(checkpoint_dir,'ckpt_map32_res'), all_loss)
...
def mc_test(file_name, number):
    ...
    with graph.as_default():
        ...
        with tf.Session(config=sess_config) as sess:
            sess.run(tf.local_variables_initializer())
#            saver.restore(sess, tf.train.latest_checkpoint('ckpt_map32_res'))
            saver.restore(sess, 'map_res/model.ckpt-33')
            ...
            try:
                ...
                save_file = open('test_map32_res.txt','a')
...
#               计算 tp,tn,fp,fn 
        ...
        #       绘制ROC曲线
                ...
                save_file.write('match fpr95 : '+str(fpr95*100)+'     match_res fpr95 : '+str(fpr95_res*100))
                save_file.write('\n')
                save_file.write('\n')
                save_file.close()
                roc_dir = 'plot_curve/roc/nature_map32_res/epoch_'+str(number)
                try:
                    os.makedirs(roc_dir)
                except os.error:
                    pass
                np.savez(roc_dir+'/match_map32_res_match_'+file_name, fpr,tpr)
                np.savez(roc_dir+'/match_map32_res_res_'+file_name, fpr_res,tpr_res)
            except KeyboardInterrupt:
                print('INTERRUPTED')
                coord.request_stop()
            finally:
